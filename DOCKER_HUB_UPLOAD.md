# Shrync v0.60 — Docker Hub upload instructie
...
```bash
chmod +x build.sh
./build.sh <jouw-dockerhub-gebruikersnaam> 0.60
```

**Voorbeeld:**
```bash
./build.sh tijmen 0.60
```

Dit doet automatisch:
1. Image bouwen voor `linux/amd64`
2. Taggen als `tijmen/shrync:latest` én `tijmen/shrync:0.60`
...
3. Verander het **Repository** veld naar:
   ```
   <jouw-gebruikersnaam>/shrync:0.60
   ```
...
docker build \
  ...
  --build-arg SHRYNC_VERSION="0.60" \
  -t <gebruikersnaam>/shrync:latest \
  -t <gebruikersnaam>/shrync:0.60 \
  .

# Pushen
docker push <gebruikersnaam>/shrync:latest
docker push <gebruikersnaam>/shrync:0.60
```
...
```bash
docker pull <gebruikersnaam>/shrync:0.60
```
