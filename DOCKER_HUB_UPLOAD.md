# Shrync v0.61 — Docker Hub upload instructie
...
```bash
chmod +x build.sh
./build.sh <jouw-dockerhub-gebruikersnaam> 0.61
```

**Voorbeeld:**
```bash
./build.sh tijmen 0.61
```

Dit doet automatisch:
1. Image bouwen voor `linux/amd64`
2. Taggen als `tijmen/shrync:latest` én `tijmen/shrync:0.61`
...
3. Verander het **Repository** veld naar:
   ```
   <jouw-gebruikersnaam>/shrync:0.61
   ```
...
docker build \
  ...
  --build-arg SHRYNC_VERSION="0.61" \
  -t <gebruikersnaam>/shrync:latest \
  -t <gebruikersnaam>/shrync:0.61 \
  .

# Pushen
docker push <gebruikersnaam>/shrync:latest
docker push <gebruikersnaam>/shrync:0.61
```
...
```bash
docker pull <gebruikersnaam>/shrync:0.61
```
